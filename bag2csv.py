...
if len(sys.argv) > 2:
	...
	print ("should be 2: 'bag2csv.py' and 'bagName' or just 1  : 'bag2csv.py'")
	sys.exit(1)

elif len(sys.argv) == 2:
	listOfBagFiles = [sys.argv[1]]
	print ("reading only 1 bagfile: {}".format(listOfBagFiles[0]))
elif len(sys.argv) == 1:
...
for bagFile in listOfBagFiles:
	count += 1
	print("Reading file {} of {}: {}".format(count, len(listOfBagFiles), bagFile))
	#access bag
	bag = rosbag.Bag(bagFile)
	bagContents = bag.read_messages()

	#create a new directory
	bagName = os.path.split(bag.filename)[-1]
	...
	bag.close()
print("Done reading all {} bag files.".format(len(listOfBagFiles)))