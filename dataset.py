...
from fssim_model import Timestep, State, Controls
from utils import tensor


class TimestepDataset(Dataset):
    def __init__(self, args):
        self.timesteps = self.get_timesteps(args.file_path, args.skip_first, args.no_samples, args.use_every)

    def get_timesteps(self, csv_file, skip, no_samples, take_every):
        timesteps = []
        ...
        for i in range(len(data[reader.fieldnames[0]])):
            ...
            timestep = Timestep(prev_state, next_state, controls, float(data['dt'][i]))
...
def collate_fn(timesteps: List[Timestep]):
    ...
    return Timestep(prev_state, next_state, controls, dt)
